...
COURSEINFO_CSV = "CourseInfo.csv"
COURSEARRANGE_CSV = "CourseArrange.csv"
...
def add_course_by_code(course_code: str,
    ...
    try:
        ...
        return "Error: 還未查到此課程（CourseInfo.csv 找不到這個課程代碼）"
    ...
    if not slots:
        return f"Error: 找到課程 {code} 但 time 欄位無法解析：{time_str!r}"
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python course_arrange_add.py <COURSE_CODE>")
        print("Example: python course_arrange_add.py F7-106")
        sys.exit(1)
