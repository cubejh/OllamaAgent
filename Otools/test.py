# course_info_tool_mapping_test.py
...
def run_once(col: str, dept_no: str, degree_label: str, headless: bool = False) -> Dict[str, Any]:
...
# 4) Pretty Print
# -----------------------------
def print_clean_result(result: Dict[str, Any]):
    if not result.get("ok"):
        print("❌ 查詢失敗")
        print("原因：", result.get("error", "unknown"))
    ...
    print(f"col：{result['fixed']['col']}   dept_no：{result['fixed']['dept_no']}   degree：{result['degree']['input']}")
    print(f"結果數量：{result.get('count', 0)}")
    print("=" * 60)
    ...
    for c in result.get("courses", []):
        ...
        print(f"系所：{c.get('系所', '')}")
        print(f"課程代碼：{c.get('課程代碼', '')}")
        print(f"課名：{c.get('課名', '')}")
        print(f"學分：{c.get('學分', '')}")
        print(f"必選修：{c.get('必選修', '')}")
        print(f"老師：{c.get('老師', '')}")
        print(f"上課時間：{ts}")
        print(f"教室：{rooms}")
        print("-" * 60)
...
# -----------------------------
# 5) Input -> mapping -> query
# -----------------------------
def parse_user_query(q: str) -> Tuple[str, str, str]:
    """
    允許輸入：
      電機資訊學院 資訊系 大二
      管理學院 資管所 3
      工學院 機械系 大4
    回傳：(college_name, dept_name, degree_label)
    """
    q = q.strip()
    q = re.sub(r"\s+", " ", q)
    parts = q.split(" ")
    if len(parts) < 3:
        raise ValueError("輸入格式請用：xx學院 xx系(或xx所) 大x（例如：電機資訊學院 資訊系 大二）")

    college_name = parts[0]
    dept_name = parts[1]
    degree_raw = parts[2]

    # 年級處理：大一/大二/大三/大四 / 1/2/3/4 / 大1/大2...
    mapping_degree = {
        "大一": "1", "大二": "2", "大三": "3", "大四": "4",
        "大1": "1", "大2": "2", "大3": "3", "大4": "4",
        "1": "1", "2": "2", "3": "3", "4": "4",
    }
    degree_label = mapping_degree.get(degree_raw, degree_raw)  # 若不在表內，就原樣送出（你也可用 "Graduate" 那種）

    return college_name, dept_name, degree_label

def map_to_codes(college_name: str, dept_name: str) -> Tuple[str, str]:
    if college_name not in COLLEGE_CODE:
        raise KeyError(f"找不到學院：{college_name}（可用：{', '.join(COLLEGE_CODE.keys())}）")

    if college_name not in DEPT_CODE:
        raise KeyError(f"此學院尚未建立系所清單：{college_name}")

    dept_map = DEPT_CODE[college_name]
    if dept_name not in dept_map:
        raise KeyError(f"找不到系所：{dept_name}（{college_name} 可用：{', '.join(dept_map.keys())}）")
...
if __name__ == "__main__":
    ...
    college_name, dept_name, degree_label = parse_user_query(q)
